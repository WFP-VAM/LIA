...
def main():
	...
	CHIRPS = xr.open_zarr(s3fs.S3Map(ODC_url.loc['CHIRPS']['url'], s3=fs))
	CHIRPS = CHIRPS.assign_coords(time = pd.to_datetime([string_to_date(x) for x in CHIRPS.time.values]))
	CHIRPS = CHIRPS.band
