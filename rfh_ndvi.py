...
def run(da_chirps, da_ndvi, shapefiles: list, wet_season: list, dry_season: list, asset_info: pd.DataFrame, path_output: str):
	...
	for i,shapefile in enumerate(shapefiles):
		...
		mean_ndvi_non = da_ndvi_out.mean(dim = ['longitude','latitude']).sel(time = t[(t.values >= pd.to_datetime(date(start_intervention[1]-1, 1, 1))) & (t.values <= pd.to_datetime(date(end_intervention[1]+1, 12, 31)))])
		mean_ndvi_ffa = da_ndvi_clipped.mean(dim = ['longitude','latitude']).sel(time = t[(da_ndvi_clipped.time.values >= pd.to_datetime(date(start_intervention[1]-1, 1, 1))) & (t.values <= pd.to_datetime(date(end_intervention[1]+1, 12, 31)))])
        
        # Maximum NDVI values (lta and not averaged)
		max_ndvi_lta = da_ndvi_clipped.groupby('time.month').mean().max(dim = ['longitude','latitude'])
		max_ndvi = da_ndvi_clipped.max(dim = ['longitude','latitude']).sel(time = t[(t.values >= pd.to_datetime(date(start_intervention[1]-1, 1, 1))) & (t.values <= pd.to_datetime(date(end_intervention[1]+1, 12, 31)))])
